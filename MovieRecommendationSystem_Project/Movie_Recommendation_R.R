# MOVIES RECOMMENDATION SYSTEM PROJECT R
...
movie_data <- read.csv("C:\\Hitesh\\MovieRecommendationSystem_Project\\IMDB-Dataset\\movies.csv",stringsAsFactors=FALSE)
rating_data <- read.csv("C:\\Hitesh\\MovieRecommendationSystem_Project\\IMDB-Dataset\\ratings.csv")
str(movie_data)
summary(movie_data)
head(movie_data)
summary(rating_data) 
...
movie_views <- colCounts(ratingMatrix) # will count views for each movie
table_views <- data.frame(movie = names(movie_views),views = movie_views) # create data frame of views
table_views <- table_views[order(table_views$views, decreasing = TRUE),] #sort by no of views
...
#HEAT MAP OF THE MOVIE RATINGS   

image(ratingMatrix[1:20, 1:25], axes = FALSE, main = "Heatmap of the first 25 rows and 20 columns")
...
image(movie_ratings[rowCounts(movie_ratings) > minimum_movies,
      ...
      main = "Hetamap of the top users and movies")
...
#Building the Recommender System on Data Set

top_recommendations <- 10 # the number of items to recommend to each user
predicted_recommendations <- predict(object = recommen_model, newdata = testing_data,
...
})  # matrix with the recommendations for each users
